...
if($queries["action"] =='update'){


    $updateSQL = sprintf("UPDATE `catch_receipt` SET `mony` = %s, `date_insert` = %s, `notes` = %s WHERE `catch_receipt`.`id` = %s;",
                             GetSQLValueString($_POST['paymentAmount'], "int"),
                             GetSQLValueString($_POST['paymentDate'], "date"),
                             GetSQLValueString($_POST['paymentNote'], "text"),
                             GetSQLValueString($_POST['inputPaymentId'], "int")
                            );
...
//     if ( !(isset($_POST["user_name"])) || !(isset($_POST["address"])) || !(isset($_POST["gender"])) || !(isset($_POST["stu_class"])) || !(isset($_POST["mobile"])) || !(isset($_POST["additional_info"])) || !(isset($_POST["userid"])) || !(isset($_POST["regdate"])) ) {
//     header("HTTP/1.1 500 Internal Server Error");
//     header('Content-Type: application/json; charset=UTF-8');
//     die(json_encode(array('message' => 'missing variables', 'code' => 1337)));
//   }

// else{
   

//         $updateSQL = sprintf("UPDATE `users` SET `user_name`=%s, `address`=%s, `reg_date`=%s, `gender`=%s, `stu_class`=%s ,`mobile`=%s , `additional_info`=%s   WHERE `user_id`=%s",
...
//           $query_student_info = sprintf("SELECT * FROM users WHERE user_id = %s", GetSQLValueString($_POST['userid'], "int"));
