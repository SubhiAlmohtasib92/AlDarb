...
$totalRows_sellected_material = mysql_num_rows($sellected_material);
  ...
  $insertSQL = sprintf("INSERT INTO courses (c_name, c_type, class_id, material_id, c_hours, c_teatcher, add_by, c_status, insert_date, c_cost) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, NOW() , %s)",
                       GetSQLValueString($row_selected_class['class_name'].' - '.$row_sellected_material['m_name'] , "text"),
...
mysql_select_db($database_conn, $conn);
$query_activ_cources = "SELECT courses.c_id, courses.c_name, courses.c_hours, users.user_name, courses.c_status, courses.insert_date, courses.c_cost  FROM courses, users WHERE courses.c_type=1 AND courses.c_teatcher = users.user_id ORDER BY courses.c_status DESC , courses.insert_date DESC";
...
$page['desc'] = 'وصف محتوى الصفحة ';
 ...
 include('templeat_header.php');
  ?>
<h3 align="center" > الدورات المنهجية  </h3> 
        ...
        <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
          <table align="center" class="table">
            <tr valign="baseline">
              ...
              <td><select  name="class_id" class="form-control" required  >
              <option></option>
              
                <?php 
do {  
?>
                <option value="<?php echo $row_all_classes['class_id']?>" ><?php echo $row_all_classes['class_name']?></option>
                <?php
} while ($row_all_classes = mysql_fetch_assoc($all_classes));
?>
              </select></td>
            </tr>
            <tr> </tr>
            <tr valign="baseline">
              <td nowrap="nowrap" align="right">المادة:</td>
              <td><select name="material_id"  class="form-control" required  >
              <option></option>
                <?php 
do {  
?>
                <option value="<?php echo $row_all_matiral['id']?>" ><?php echo $row_all_matiral['m_name']?></option>
                <?php
} while ($row_all_matiral = mysql_fetch_assoc($all_matiral));
?>
              </select>
              
              <a href="admin_matiral.php" > ادارة المواد </a>
...
?>
            ...
            <tr valign="baseline">
              ...
              <td><input type="submit" value="  فتح شعبة دورة منهجية " /></td>
            </tr>
          </table>
